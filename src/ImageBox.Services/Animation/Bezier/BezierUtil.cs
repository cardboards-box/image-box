...
public static class BezierUtil
{
    ...
    public static BoxPoint BezierPoint(double t, BoxPoint[] controlPoints)
    {
        ...
        BoxPoint[] points = new BoxPoint[n + 1];
    ...
    /// Calculate the point on the curve at the given time
    /// </summary>
    /// <param name="scope">The scope to calculate the point from</param>
    /// <param name="points">The bezier points to use</param>
    /// <param name="bezier">The type of bezier curve</param>
    /// <param name="easing">The easing function to apply to the curve</param>
    /// <returns>The point in the bezier curve</returns>
    public static BezierPoint Calculate(ContextScope scope, BoxUnitPoint[] points, BezierType bezier, EasingType easing)
    {
        double t = scope.Frame.Frame / (double)scope.Frame.TotalFrames;
        var controlPoints = points.Select(p => p.Pixels(scope.Size)).ToArray();
        return Calculate(t, controlPoints, bezier, easing);
    }
}
