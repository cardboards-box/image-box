    ...
    ILogger<ElementReflectionService> _logger) : IElementReflectionService
{
    ...
    /// <param name="index">The index of the current element in relation to it's siblings</param>
    ...
    public IElement? BindInstance(ReflectedElement type, AstElement element, bool skipChildren, IElement? parent, int index)
    {
        ...
        instance.SiblingIndex = index;
        //Find and set any child elements on this template
        if (type.ChildType == AstElementType.Children &&
            element.Children.Length > 0 &&
            !skipChildren)
        {
            var children = BindTemplates(element.Children, skipChildren, instance).ToArray();
    ...
    public IEnumerable<IElement> BindTemplates(IEnumerable<AstElement> elements, bool skipChildren, IElement? parent)
    {
        ...
        foreach (var element in elements)
        {
            ...
            var instance = BindInstance(type, element, skipChildren, parent, index);
